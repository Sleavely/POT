...
class POTTest extends PHPUnit_Framework_TestCase {
    ...
    private $config = array('host' => 'localhost', 'user' => 'wrzasq', 'database' => 'otserv');
    ...
    public function testGetInstance() {
        $ots = POT::getInstance();
        $this->assertEquals(true, $ots === $this->ots);
    }

    public function testLoadClass() {
        try
        {
            $this->ots->loadClass('IOTS_DB');
            $this->assertEquals(true, interface_exists('IOTS_DB') );
            $this->ots->loadClass('Foo');
        }
        catch(Exception $e)
        {
            $this->assertEquals('\'Foo\' is not part of PHP OTS Toolkit.', $e->getMessage() );
        }
    }

    public function testConnect() {
        $this->ots->loadClass('IOTS_DB');
        $this->ots->loadClass('OTS_DB_MySQL');
        $this->ots->loadClass('OTS_DB_SQLite');
        $this->ots->loadClass('OTS_SQLite_Results');

        $this->ots->connect(POT::DB_MYSQL, $this->config);
        $this->ots->connect(POT::DB_SQLITE, $this->config);

        try
        {
            $this->ots->connect('Foo', $this->config);
        }
        catch(Exception $e)
        {
            $this->assertEquals('Driver \'Foo\' is not supported.', $e->getMessage() );
        }
    }

    public function testCreateObject() {
        $this->ots->loadClass('IOTS_DB');
        $this->ots->loadClass('IOTS_DAO');
        $this->ots->loadClass('OTS_DB_SQLite');
        $this->ots->loadClass('OTS_SQLite_Results');
        $this->ots->loadClass('OTS_Account');
        $this->ots->loadClass('OTS_Player');
